...
def cost_usd(tokens: Mapping[str, int], model: str = MODEL) -> float:
    prices = PRICE_PER_MILLION_TOKENS[model]
    return sum(tokens[field] * prices[field] for field in TOKEN_FIELDS) / 1_000_000


def record_usage(db: Database, conversation_id: str, steps: Sequence[TraceStep], outcome: str) -> None:
